  ...
  <v-container fluid :style="{ height: suggestedHeight }" class="pa-0">
    <v-row style="height: 100%">
      <v-col cols="3" class="overflow-y-auto" style="height: 100%">
        <v-card outlined class="mb-2" :class="{ 'warning-outline': baselineRepoId === null }">
          <v-card-text>
      ...
      <v-col cols="9" style="height: 100%">
        <v-card outlined class="mb-2" style="height: 64px">
          <v-card-text class="d-flex justify-end align-center py-0" style="height: 100%">
...
export default class StatusComparison extends Vue {
  ...
  private get allDimensions() {
    return vxm.repoModule.occuringDimensions(vxm.repoModule.allRepos.map((it) => it.id));
  }
  ...
  private mapEquals(first: Map<RepoId, string[]>, second: Map<RepoId, string[]>) {
