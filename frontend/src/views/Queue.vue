    ...
    <v-container>
      <v-row align="baseline" justify="center">
        <v-col>
          <v-card>
            <v-card-title>
              <v-toolbar color="toolbarColor" dark> Available Runners </v-toolbar>
            </v-card-title>
            <v-card-text>
              <v-container fluid>
                <v-row align="center">
              ...
              <v-container fluid class="ma-0 pa-0">
                <v-row justify="end" no-gutters>
                  <v-col cols="auto" class="mb-5" v-if="isAdmin">
                    <upload-tar-dialog v-model="uploadTarDialogOpen"></upload-tar-dialog>
                    <v-btn color="primary" text outlined @click="uploadTarDialogOpen = true">
                      Upload Tar
                    </v-btn>
                  </v-col>
                  <v-spacer></v-spacer>
                  <v-col cols="auto" class="mb-5" v-if="isAdmin">
                    <v-tooltip bottom>
                      ...
                      Executes a `git fetch` updating the benchmark repo as well all other repos. It
                      should find any new commits and pick up changes to the benchmark repo, but
                  ...
                  <v-col cols="auto" class="mb-5" v-if="isAdmin">
                    <v-tooltip left>
                      <template #activator="{ on }">
                        <v-btn v-on="on" color="warning" text outlined @click="cancelAllTasks()">
                          Cancel all tasks
                        </v-btn>
                      </template>
                      Cancels
...
export default class Queue extends Vue {
  ...
  private async refetchRepos() {
    ...
    this.$globalSnackbar.setSuccess("listener", "Re-fetched repo and updated benchrepo", 2);
  }
