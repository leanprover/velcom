...
export default class CommitBenchmarkActions extends Vue {
    ...
    return [
      {
        ...
        icon: this.hasExistingBenchmark ? this.rebenchmarkIcon : this.benchmarkIcon,
  ...
  private async benchmarkUpwards() {
    const taskCount = await vxm.queueModule.dispatchQueueUpwardsOf(this.commitDescription);
  ...
  private get commitRemoteLink(): string | undefined {
    ...
    const match = /^(https:\/\/(.+)\/|git@(.+):)(.+)\/(.+?)(\.git)?$/.exec(this.repo.remoteURL);
    if (!match) {
      return undefined;
    }
    ...
    const commitMarker = this.repo.remoteURL.includes("bitbucket") ? "commits" : "commit";
