<template>
  <v-snackbar shaped v-model="displaySnackbar" :timeout="timeout" :color="color">
    <v-progress-circular class="mr-2" v-if="loading" indeterminate></v-progress-circular>
    {{ text }}
    <template v-slot:action="{ attrs }">
      <v-btn dark color="ping" text v-bind="attrs" @click="displaySnackbar = false">Close</v-btn>
    </template>
  </v-snackbar>
</template>
...
export default class Snackbar extends Vue implements ISnackbar {
  ...
  private displayNormalText(text: string, color: Color, timeout: number, priority: number = 1) {
