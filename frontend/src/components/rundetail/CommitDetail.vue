          ...
          <v-card-title>
            <v-toolbar dark color="toolbarColor" class="wrapping-toolbar">
              <v-container fluid class="ma-0 pa-0">
                <v-row no-gutters align="center" justify="center" justify-lg="space-between">
                  <v-col cols="auto">
                    <router-link
...
class NavigationTarget {
  ...
  constructor(parent: TrackedCommitDescription | null, child: TrackedCommitDescription | null) {
    this.parent = parent;
    this.child = child;
  }
}
...
export default class CommitDetail extends Vue {
  ...
  private get navigationTargets(): NavigationTarget[] {
    ...
    const mutualItems = Math.min(this.commit.parents.length, this.commit.children.length);
    let i = 0;
    for (; i < mutualItems; i++) {
      targets.push(new NavigationTarget(this.commit.parents[i], this.commit.children[i]));
    }
