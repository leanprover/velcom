<template>
  <v-card>
    ...
    <v-card-text>
      <v-alert :type="taskInProgress ? 'error' : 'warning'" class="mx-4" v-if="loadingError">
        ...
        <span v-if="!taskInQueue">
          It looks like the task is no longer in the queue? Maybe it is finished?
        </span>
      </v-alert>
      <div class="runner-output mx-2">
        <div v-for="{ lineNumber, text } in lines" :key="lineNumber" class="line">
          <span
