<template>
  <v-card
    ...
    style="height: 100%"
  >
    <v-card-text :class="{ disabled: errorMessage !== null }" style="height: 100%">
      <v-row class="my-0" align="center" style="height: 100%">
        <v-col v-if="errorMessage !== null" class="d-flex align-center text-warning py-0">
          {{ errorMessage }}
        </v-col>
...
export default class ComparisonDimensionSelector extends Vue {
  ...
  private get selectedItem() {
    ...
    if (!this.possibleDimensions.find((it) => it.equals(this.selectedDimension))) {
      return null;
    }
    return this.items.find((it) => it.dimension.equals(this.selectedDimension)) || null;
  }
