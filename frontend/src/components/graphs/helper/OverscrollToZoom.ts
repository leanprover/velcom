...
export default class OverscrollToZoom {
  ...
  private isZoomedOut(): boolean {
    if (this.store.zoomXStartValue == null && this.store.zoomXEndValue == null) {
      return true;
    }
  ...
  public scrolled(e: WheelEvent): void {
    ...
    this.store.startTime = new Date(this.store.startTime.getTime() - 7 * 1000 * 60 * 60 * 24);

    const clamp = (a: number) => Math.min(a, new Date().getTime());
    this.store.endTime = new Date(clamp(this.store.endTime.getTime() + 7 * 1000 * 60 * 60 * 24));
