                  ...
                  >
                    <template #activator="{ on }">
                      <v-text-field
                        ...
                        :rules="field.rules"
                      >
                        <v-icon slot="append" @click="lockDates(field.role)" class="lock-button">
                    ...
                    <v-date-picker
                      ...
                      :event-color="(date) => (date === startTimeString ? 'blue' : 'red')"
                    >
                      <v-btn text color="primary" @click="field.saveFunction(field, today)">
                        Today
                      </v-btn>
                      <v-spacer></v-spacer>
                      <v-btn text color="primary" @click="field.opened = false"> Cancel </v-btn>
                      <v-btn text color="primary" @click="field.saveFunction(field, field.value)">
                        OK
                      </v-btn>
                    </v-date-picker>
                  </v-menu>
...
export default class GraphTimespanControls extends Vue {
  ...
  private saveDuration() {
    ...
    if (this.dateLocked === "start") {
      this.$emit("update:endTime", new Date(this.startTime.getTime() + durationAsMillis));
    } else {
      this.$emit("update:startTime", new Date(this.endTime.getTime() - durationAsMillis));
    }
