...
type DatapointErrorType = "NO_RUN" | "MEASUREMENT_FAILED" | "RUN_FAILED" | "NO_MEASUREMENT";
...
export default class StatusComparisonGraph extends Vue {
  ...
  private get processedDataPoints(): Map<string, Datapoint[] | DatapointRepoError> {
  ...
  private pointsForRun(run: Run, repoId: RepoId): DatapointRepoError | Datapoint[] {
    ...
    this.chartOptions = {
      ...
      toolbox: {
        ...
        feature: {
          ...
          myDecal: {
            show: true,
            title: this.showDecals ? "Hide pattern in bars" : "Show pattern in bars",
            icon: mdiDotsHexagon,
            onclick: () => {
  ...
  private generateSeries(repoId: RepoId): BarSeriesOption & { name: string } {
    ...
    const seriesData = this.selectedDimensions
      ...
      .map((dim) => {
        const displayedPoint = datapoints.find((point) => dimensionIdEquals(dim, point.dimension));
  ...
  private datapointClicked(e: any) {
    ...
    const originalDatapoint = this.datapoints.find((it) => it.repoId === echartsPoint.repoId);
