  ...
  <v-container fluid class="mt-0 pt-0 full-height">
    <v-row align="center" justify="center" class="full-height" no-gutters>
      <v-col class="full-height">
        ...
        ></slot>
        <div id="chart-container" @wheel.capture="$emit('wheel', $event)" class="full-height">
          <v-chart
            ref="chart"
            :autoresize="true"
            :option="chartOptions"
...
import { AttributedDatapoint, GraphDataPoint, SeriesId, SeriesInformation } from "@/store/types";
import { Prop, Watch } from "vue-property-decorator";
import EChartsComp from "vue-echarts";

import { LineChart, LineSeriesOption, GraphChart, GraphSeriesOption } from "echarts/charts";
...
type SeriesGenerationFunction = (seriesInformation: SeriesInformation) => ValidEchartsSeries;
...
export default class EchartsDetailGraph extends Vue {
  ...
  private tooltipPositioner(point: [number, number], _: unknown, el: unknown) {
    if (!(el instanceof HTMLDivElement)) {
      ...
      throw new Error("Only HTML tooltips are supported in our custom tooltip position!");
    }
  ...
  private getHighestPointValue(point: GraphDataPoint): number {
    return Math.max(
      0,
      ...Array.from(point.values.values()).filter((v): v is number => typeof v === "number"),
    );
  }
  ...
  private findFirstSuccessful(seriesId: SeriesId) {
    ...
    if (point) {
      return [point.values.get(seriesId) as number, this.getHighestPointValue(point)];
    }
    return [0, 0];
  }
  ...
  private buildLineSeries(series: SeriesInformation): LineSeriesOption {
    ...
    const echartPoints: EchartsDataPoint[] = this.echartsDataPoints.get(series.id)!;
  ...
  private updateMarkPoints() {
    ...
    if (this.showReferenceMarkers) {
      ...
      if (displayedPoint) {
        markPointData.push({
          coord: [displayedPoint.positionTime, point.values.get(this.referenceDatapoint!.seriesId)],
          label: {
            show: true,
            position: "inside",
            formatter: () => "R",
