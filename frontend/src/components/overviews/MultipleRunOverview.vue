...
export default class MultipleRunOverview extends Vue {
  ...
  private run(run: RunDescription | RunDescriptionWithDifferences): RunDescription {
  ...
  private significantDifferences(
    ...
    return run instanceof RunDescriptionWithDifferences ? run.significantDifferences : [];
  }

  private failedSignificantDimensions(
    run: RunDescription | RunDescriptionWithDifferences,
  ): Dimension[] {
    return run instanceof RunDescriptionWithDifferences ? run.significantFailedDimensions : [];
  }
}
</script>
