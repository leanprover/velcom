...
export function groupBy<K, V>(list: V[], keyGetter: (value: V) => K): Map<K, V[]> {
  const map = new Map();
  list.forEach((item) => {
...
export function spaceDayEquidistant<T extends GraphDataPoint>(detailGraph: T[]): T[] {
  ...
  return groupEntries.flatMap(([day, points]) => {
    ...
    return points.map((point, index) => {
      return point.positionedAt(new Date(day + spacingBetweenElementsMillis * index)) as T;
    });
  });
}
