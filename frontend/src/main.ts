...
axios.interceptors.response.use(
  (response) => {
    ...
    vue.$globalSnackbar.finishedLoading(prefix, response.config.snackbarPriority);

    if (response.config.showSuccessSnackbar) {
      vue.$globalSnackbar.setSuccess(prefix, "Success!", response.config.snackbarPriority);
    }
