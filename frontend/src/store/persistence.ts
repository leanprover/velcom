...
export const persistenceSessionStorage = new VuexPersistence<Partial<RootState>>({
  storage: window.sessionStorage,
  saveState: (key, rawState, storage) => {
    const state = rawState as RootState;

    const persistable = {
      detailGraphModule: DetailGraphStore.toPlainObject(state.detailGraphModule),
      comparisonGraphModule: ComparisonGraphStore.toPlainObject(state.comparisonGraphModule),
      statusComparisonModule: StatusComparisonStore.toPlainObject(state.statusComparisonModule),
    };
