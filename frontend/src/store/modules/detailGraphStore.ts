...
import { orElse, orUndefined, parseAndSetZoomAndDateRange, respectOptions } from "@/util/LinkUtils";
...
export class DetailGraphStore extends VxModule {
  ...
  async fetchDetailGraph(): Promise<DetailDataPoint[]> {
    ...
    const effectiveStartTime: number = Math.floor(this.bufferedTimespan.startTime.getTime() / 1000);
    ...
    const seriesIds = response.data.dimensions
      ...
      .map((dim: Dimension) =>
        this._selectedDimensions.find((savedDimension) => savedDimension.equals(dim)),
      )
      .map((it: Dimension) => it.toString());

    const dataPoints: DetailDataPoint[] = response.data.commits.map((json: any) =>
  ...
  async adjustToPermanentLink(link: Route): Promise<void> {
    ...
    const repo = vxm.repoModule.repoById(repoId) || (await vxm.repoModule.fetchRepoById(repoId));
  ...
  get bufferedTimespan(): { startTime: Date; endTime: Date } {
    ...
    const bufferedStartTime = new Date(this.startTime.getTime() - bufferMillis / 2);
  ...
  get detailGraph(): DetailDataPoint[] {
    const unbufferedGraph = this._detailGraph.filter(
      (datapoint: DetailDataPoint) =>
        datapoint.committerTime >= this.startTime && datapoint.committerTime <= this.endTime,
    );

    return this.dayEquidistantGraph ? spaceDayEquidistant(unbufferedGraph) : unbufferedGraph;
  }
  ...
  set selectedRepoId(selectedRepoId: RepoId) {
    ...
    if (repo) {
      ...
      vxm.detailGraphModule.selectedDimensions = this._selectedDimensions.filter(isInRepo);
    } else {
      // we need to trigger the setter
  ...
  get permanentLink(): (options?: PermanentLinkOptions) => string {
    return (options) => {
      const route = router.resolve({
        ...
        query: {
          zoomYStart: respectOptions(options, "includeYZoom", orUndefined(this.zoomYStartValue)),
          zoomYEnd: respectOptions(options, "includeYZoom", orUndefined(this.zoomYEndValue)),
          zoomXStart:
