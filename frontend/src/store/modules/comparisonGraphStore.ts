...
import { AttributedDatapoint, ComparisonDataPoint, Dimension, RepoId } from "@/store/types";
import Vue from "vue";
...
function defaultStartDate() {
  ...
  return new Date(new Date(new Date().setDate(new Date().getDate() - 14)).setHours(0, 0, 0, 0));
}
...
export class ComparisonGraphStore extends VxModule {
  ...
  async fetchComparisonGraph(): Promise<ComparisonDataPoint[]> {
    ...
    const response = await axios.get("/graph/comparison", {
      params: {
        ...
        dimension: this.selectedDimension.benchmark + ":" + this.selectedDimension.metric,
  ...
  @mutation
  setSelectedBranchesForRepo(payload: { repoId: RepoId; branches: string[] }): void {
        ...
        query: {
          ...
          dimension: respectOptions(
            ...
            this.selectedDimension
              ? this.selectedDimension.benchmark + ":" + this.selectedDimension.metric
              : undefined,
          ),
