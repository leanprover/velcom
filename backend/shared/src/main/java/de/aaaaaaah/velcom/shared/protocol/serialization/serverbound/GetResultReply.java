...
public class GetResultReply implements ServerBound {
  ...
  @Nullable private final Result result;
  ...
  public GetResultReply(
    ...
    if (success && (result == null || error != null)) {
      throw new IllegalArgumentException("if successful, there must be a result and no error");
    } else if (!success && (result != null || error == null)) {
      throw new IllegalArgumentException("if not successful, there must be an error and no result");
    }
  ...
  public ServerBoundPacket asPacket(Serializer serializer) {
    return new ServerBoundPacket(
        ServerBoundPacketType.GET_RESULT_REPLY, serializer.serializeTree(this));
  }
}
