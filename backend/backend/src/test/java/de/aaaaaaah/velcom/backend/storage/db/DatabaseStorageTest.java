...
class DatabaseStorageTest {
  ...
  void canWriteNewInfo(@TempDir Path tempDir) {
    ...
    databaseStorage.acquireWriteTransaction(
        db -> {
          db.dsl()
              .batchInsert(new RepoRecord(repoId2, repoName2, repoRemoteUrl, null, null))
              .execute();
        });
    ...
    databaseStorage.acquireWriteTransaction(
        db -> {
          db.dsl()
              .batchInsert(new RepoRecord(repoId3, repoName3, repoRemoteUrl, null, null))
              .execute();
          return true;
        });
    ...
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      Map<String, RepoRecord> records =
          db.dsl().selectFrom(REPO).stream().collect(toMap(RepoRecord::getId, it -> it));
