...
public class TestDb {
  ...
  public void addRepo(RepoId repoId, String name, RemoteUrl remoteUrl) {
    dslContext
        .batchInsert(new RepoRecord(repoId.getIdAsString(), name, remoteUrl.getUrl(), null, null))
        .execute();
  }
  ...
  public void addBranch(RepoId repoId, BranchName name, CommitHash commitHash, boolean tracked) {
    dslContext
        .batchInsert(
            new BranchRecord(repoId.getIdAsString(), name.getName(), commitHash.getHash(), tracked))
        .execute();
  }
  ...
  public void addCommitRel(RepoId repoId, CommitHash parent, CommitHash child) {
    dslContext
        .batchInsert(
            new CommitRelationshipRecord(repoId.getIdAsString(), parent.getHash(), child.getHash()))
        .execute();
  }
    ...
    dslContext
        .batchInsert(
            new RunRecord(
                ...
                stopTime,
                source
                    .consume(cs -> Optional.of(cs.getRepoId()), TarSource::getRepoId)
                    .map(RepoId::getIdAsString)
                    .orElse(null),
                source.getLeft().map(CommitSource::getHash).map(CommitHash::getHash).orElse(null),
                source.getRight().map(TarSource::getDescription).orElse(null),
                Optional.ofNullable(error)
                    .map(RunError::getType)
                    .map(RunErrorType::getTextualRepresentation)
                    .orElse(null),
                Optional.ofNullable(error).map(RunError::getMessage).orElse(null)))
        .execute();
  }
  ...
  public UUID addMeasurement(
      RunId runId,
      Dimension dimension,
      @Nullable Unit unit,
      @Nullable Interpretation interpretation,
      Either<MeasurementError, MeasurementValues> result) {
    ...
    dslContext
        .batchInsert(
            new MeasurementRecord(
                ...
                dimension.getMetric(),
                Optional.ofNullable(unit).map(Unit::getName).orElse(null),
                Optional.ofNullable(interpretation)
                    .map(Interpretation::getTextualRepresentation)
                    .orElse(null),
                result.getLeft().map(MeasurementError::getErrorMessage).orElse(null)))
        .execute();
                ...
                dslContext
                    .batchInsert(
                        values.getValues().stream()
                            .map(
                                value ->
                                    new MeasurementValueRecord(measurementId.toString(), value))
                            .collect(toList()))
                    .execute());
