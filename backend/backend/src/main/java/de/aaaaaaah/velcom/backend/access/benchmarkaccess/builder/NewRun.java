...
public class NewRun {
  ...
  public Optional<RepoId> getRepoId() {
    return getSource()
        .consume(commitSource -> Optional.of(commitSource.getRepoId()), TarSource::getRepoId);
  }
  ...
  public Run toRun() {
    Either<RunError, Collection<Measurement>> result =
        this.result.mapRight(
            newMeasurements ->
                newMeasurements.stream().map(NewMeasurement::toMeasurement).collect(toList()));
