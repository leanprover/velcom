...
public class BenchmarkReadAccess {
  ...
  private List<Run> loadRuns(DBReadAccess db, List<RunRecord> runRecords) {
    ...
    // 0. Preparation
    Set<String> runIds = runRecords.stream().map(RunRecord::getId).collect(toSet());
    ...
    // 2. Load measurements
    Map<RunId, List<Measurement>> measurements =
        db.dsl().selectFrom(MEASUREMENT).where(MEASUREMENT.RUN_ID.in(runIds)).stream()
            .map(
                record -> {
        ...
        .map(
            record -> {
              ...
              final Either<CommitSource, TarSource> source =
                  AccessUtils.readSource(
                      record.getRepoId(), record.getCommitHash(), record.getTarDesc());
  ...
  public List<Run> getRuns(Collection<RunId> runIds) {
    Set<String> runIdStrings = runIds.stream().map(RunId::getIdAsString).collect(toSet());

    return databaseStorage.acquireReadTransaction(
        db -> {
          List<RunRecord> runRecords =
              db.dsl().selectFrom(RUN).where(RUN.ID.in(runIdStrings)).stream().collect(toList());
   ...
   * associated commit are ignored. If a commit has multiple runs, they are ordered from new to old.
   *
  ...
  public List<RunId> getRecentRunIds(int skip, int amount) throws IllegalArgumentException {
    ...
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      return db
          ...
          .from(RUN)
          .join(KNOWN_COMMIT)
          .on(KNOWN_COMMIT.REPO_ID.eq(RUN.REPO_ID).and(KNOWN_COMMIT.HASH.eq(RUN.COMMIT_HASH)))
  ...
  public Map<CommitHash, RunId> getLatestRunIds(
    ...
    Set<String> hashStrings = commitHashes.stream().map(CommitHash::getHash).collect(toSet());
  ...
  public List<SearchRunDescription> searchRuns(
      Integer limit, @Nullable RepoId repoId, String queryStr) {
    ...
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      ...
      return query.orderBy(RUN.START_TIME.desc()).limit(limit).stream()
          .map(
              record ->
                  new SearchRunDescription(
  ...
  public ShortRunDescription getShortRunDescription(RunId runId) throws NoSuchRunException {
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      return db.dsl()
          ...
          .map(
              record ->
                  new ShortRunDescription(runId, record.value1(), record.value2(), record.value3()))
  ...
  public long getAmountOfOutdatedRuns() {
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      ...
      return db.dsl()
          ...
          .from(
              select(count().minus(1).as(namedField))
                  .from(RUN)
  ...
  public long getAmountOfFailedRuns() {
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      return db.dsl().select(count()).from(RUN).where(RUN.ERROR.isNotNull()).fetchSingle().value1();
    }
  }
