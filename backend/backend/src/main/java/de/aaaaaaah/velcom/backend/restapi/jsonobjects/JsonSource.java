...
public class JsonSource {
  ...
  @Nullable private final JsonCommitDescription commitDescription;
  @Nullable private final JsonSourceUploadedTar uploadedTar;

  private JsonSource(
      JsonSourceType type,
      @Nullable JsonCommitDescription commitDescription,
  ...
  public static JsonSource tarSource(String description, @Nullable UUID repoId) {
    return new JsonSource(
        JsonSourceType.UPLOADED_TAR, null, new JsonSourceUploadedTar(description, repoId));
  }
  ...
  public static JsonSource fromTarSource(TarSource source) {
    return tarSource(source.getDescription(), source.getRepoId().map(RepoId::getId).orElse(null));
  }

  public static JsonSource fromSource(
      Either<CommitSource, TarSource> source, CommitReadAccess commitAccess) {
  ...
  private static class JsonSourceUploadedTar {
    ...
    @Nullable private final UUID repoId;
