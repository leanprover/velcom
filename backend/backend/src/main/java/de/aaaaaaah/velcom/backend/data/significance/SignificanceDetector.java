...
public class SignificanceDetector {
  ...
  public SignificanceDetector(
      SignificanceFactors significanceFactors, RunComparator runComparator) {
   ...
   */
  public Optional<SignificanceReasons> getSignificance(
      Run run, Collection<Run> comparedTo, Set<Dimension> significantDimensions) {
    ...
    List<Dimension> significantFailedDimensions =
        run.getResult().getRight().get().stream()
    ...
    if (!significantDifferences.isEmpty() || !significantFailedDimensions.isEmpty()) {
      return Optional.of(
          new SignificanceReasons(significantDifferences, significantFailedDimensions, false));
    } else {
  ...
  private boolean isSignificantDifference(DimensionDifference diff) {
    ...
    boolean relSignificant =
        diff.getReldiff()
            .map(reldiff -> Math.abs(reldiff) >= significanceFactors.getRelativeThreshold())
            // There is no reldiff if the first value is 0. But if the second value is also zero,
            // that
            // hardly constitutes a significant difference. Otherwise, it is a move away from 0,
            // which is
            // always significant.
            .orElse(diff.getFirst() != diff.getSecond());

    boolean stddevSignificant =
        diff.getSecondStddev()
            .map(
                stddev ->
                    Math.abs(diff.getDiff()) >= significanceFactors.getStddevThreshold() * stddev)
            // If there is no stddev, this check should not prevent differences from being
            // significant
            .orElse(true);
