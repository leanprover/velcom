...
public class RepoReadAccess {
  ...
  public List<Repo> getAllRepos() {
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      return db.dsl().fetch(REPO).stream().map(RepoReadAccess::repoRecordToRepo).collect(toList());
    }
  }
  ...
  public List<Branch> getAllBranches(RepoId repoId) {
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      return db.dsl().fetch(BRANCH, BRANCH.REPO_ID.eq(repoId.getIdAsString())).stream()
          .map(RepoReadAccess::branchRecordToBranch)
          .collect(toList());
    }
  }

  public List<SearchBranchDescription> searchBranches(
