...
public class Queue {
   ...
   *     in no particular order. Then come the tasks which are currently not in progress, in the
   *     order they would be returned by successive calls to {@link #startNextTask()}.
   */
  public List<Task> getAllTasksInOrder() {
    ...
    List<Task> tasksNotInProgress =
        allTasks.stream().filter(task -> !task.isInProgress()).collect(toList());
   ...
   */
  public Optional<Task> addCommit(
      String author, RepoId repoId, CommitHash hash, TaskPriority priority) {
  ...
  public void addCommits(
      String author, RepoId repoId, List<CommitHash> hashes, TaskPriority priority) {
