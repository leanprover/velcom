package de.aaaaaaah.velcom.backend.access.archiveaccess;
...
public class ArchiveReadAccess {
  ...
  public ArchiveReadAccess(
      Path rootDir, RepoStorage repoStorage, TarFileStorage tarFileStorage, String benchRepoUrl)
      throws IOException {
  ...
  /**
   * Transfer the tar file for a task to an {@link OutputStream}. Creates the tar file if necessary.
   *
  ...
  private void transferCommitTask(Task task, CommitSource commitSource, OutputStream outputStream)
    ...
    transferCommit(
        task, commitSource.getRepoId().getDirectoryName(), commitSource.getHash(), outputStream);
  }

  private void transferCommit(
      @Nullable Task task, String repoDirName, CommitHash commitHash, OutputStream outputStream)
      throws TarRetrieveException, TarTransferException {
