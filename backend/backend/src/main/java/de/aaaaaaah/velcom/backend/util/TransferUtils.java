...
public class TransferUtils {
  ...
  public static void cloneRepo(
      RepoStorage repoStorage, String dirName, Path destDir, CommitHash hash)
    ...
    try (Repository repository = repoStorage.acquireRepository(dirName)) {
      ...
      Objects.requireNonNull(
          commitId, "unknown commit hash " + hash + " for local repo " + dirName);
      ...
      GuickCloning.getInstance()
          .cloneCommit(originalRepoPath.toAbsolutePath().toString(), destDir, hash.getHash());
    } catch (Exception e) {
