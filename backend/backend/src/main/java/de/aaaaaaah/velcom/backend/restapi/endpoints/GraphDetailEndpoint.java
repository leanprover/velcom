...
public class GraphDetailEndpoint {
  ...
  public GetReply get(
    ...
    List<DimensionInfo> existingDimensions =
    ...
    Map<CommitHash, Run> runs =
        latestRunCache.getLatestRuns(benchmarkAccess, runCache, repoId, fullCommitsByHash.keySet());
    ...
    List<JsonDimension> jsonDimensions =
        existingDimensions.stream()
            ...
            .collect(Collectors.toList());
    List<JsonGraphCommit> jsonGraphCommits =
        fullCommits.stream()
            .map(
                commit ->
                    new JsonGraphCommit(
  ...
  private static List<Object> extractValuesFromCommit(
      List<DimensionInfo> dimensions, Map<CommitHash, Run> runs, Commit commit) {
    ...
    Map<Dimension, Measurement> measurements =
        optionalMeasurements.get().stream().collect(toMap(Measurement::getDimension, m -> m));
  ...
  private static List<Object> createListFullOf(List<DimensionInfo> dimensions, String string) {
    return dimensions.stream().map(dim -> string).collect(Collectors.toList());
  }
