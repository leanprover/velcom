...
public class BenchmarkWriteAccess extends BenchmarkReadAccess {
  ...
  public void insertRun(NewRun newRun) {
    ...
    newRun
        ...
        .ifPresent(
            commitSource ->
                latestRunCache.invalidate(commitSource.getRepoId(), commitSource.getHash()));
  }

  private void deleteTask(DBWriteAccess db, TaskId taskId) {
    db.dsl().deleteFrom(TASK).where(TASK.ID.eq(taskId.getIdAsString())).execute();
  }

  private void updateDimensions(DBWriteAccess db, NewRun newRun) {
    Map<Dimension, DimensionRecord> dimensions =
        db.dsl().selectFrom(DIMENSION).stream()
            .collect(
                toMap(
                    record -> new Dimension(record.getBenchmark(), record.getMetric()), it -> it));
  ...
  private void insertNewRun(DBWriteAccess db, NewRun run) {
    RunRecord runRecord =
        new RunRecord(
            ...
            run.getStartTime(),
            run.getStopTime(),
            run.getRepoId().map(RepoId::getIdAsString).orElse(null),
            run.getSource()
                .getLeft()
                .map(CommitSource::getHash)
                .map(CommitHash::getHash)
                .orElse(null),
            run.getSource().getRight().map(TarSource::getDescription).orElse(null),
            run.getResult()
                .getLeft()
                .map(RunError::getType)
                .map(RunErrorType::getTextualRepresentation)
                .orElse(null),
            run.getResult().getLeft().map(RunError::getMessage).orElse(null));
  ...
  private void insertNewMeasurement(DBWriteAccess db, NewMeasurement measurement) {
    ...
    MeasurementRecord measurementRecord =
        new MeasurementRecord(
            ...
            measurement.getUnit().map(Unit::getName).orElse(null),
            measurement
                .getInterpretation()
                .map(Interpretation::getTextualRepresentation)
                .orElse(null),
            measurement.getContent().getLeft().map(MeasurementError::getErrorMessage).orElse(null));

    db.dsl().batchInsert(measurementRecord).execute();

    List<MeasurementValueRecord> records =
        measurement.getContent().getRight().stream()
            .flatMap(
                values ->
