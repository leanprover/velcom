...
public class SignificantRunsCollector {
  ...
  public Optional<SignificantRun> getSignificantRun(Run run) {
    ...
    Set<CommitHash> parentHashes =
        commitAccess.getParentHashes(source.getRepoId(), source.getHash());
    Collection<Run> parentRuns =
        latestRunCache
  ...
  private Map<CommitSource, Collection<Run>> getParentRunsPerSource(
    ...
    commitsPerRepo.forEach(
        (repoId, commits) -> {
          Set<CommitHash> parentHashes =
              commits.stream()
                  .flatMap(commit -> commit.getParentHashes().stream())
                  .collect(toSet());
          Map<CommitHash, Run> latestRuns =
              latestRunCache.getLatestRuns(benchmarkAccess, runCache, repoId, parentHashes);
    ...
    return commitsPerRepo.values().stream()
        ...
        .collect(
            toMap(
                ...
                commit -> {
                  // There will always be an entry for the repo id as long as commitsPerRepo doesn't
                  // contain
   ...
   */
  private Optional<SignificantRun> getSignificantRun(
      Run run, Map<CommitSource, Collection<Run>> parentMap, Set<Dimension> significantDimensions) {

    return run.getSource()
        .getLeft()
        .map(source -> Optional.ofNullable(parentMap.get(source)).orElse(List.of()))
        .flatMap(
            parents -> significanceDetector.getSignificance(run, parents, significantDimensions))
