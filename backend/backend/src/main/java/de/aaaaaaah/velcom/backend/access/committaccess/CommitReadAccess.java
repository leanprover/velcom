...
public class CommitReadAccess {
  ...
  private static FullCommit knownCommitRecordToFullCommit(
      KnownCommitRecord record, Set<CommitHash> parentHashes, Set<CommitHash> childHashes) {
  ...
  private static FullCommit commitToFullCommit(
      Commit commit, Set<CommitHash> parentHashes, Set<CommitHash> childHashes) {
  ...
  public Commit getCommit(RepoId repoId, CommitHash commitHash) throws NoSuchCommitException {
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      KnownCommitRecord record =
          db.dsl()
              .fetchSingle(
                  KNOWN_COMMIT,
                  KNOWN_COMMIT
                      .REPO_ID
                      .eq(repoId.getIdAsString())
  ...
  public List<Commit> getCommits(RepoId repoId, Collection<CommitHash> commitHashes) {
    Set<String> hashes = commitHashes.stream().map(CommitHash::getHash).collect(toSet());
  ...
  public List<FullCommit> promoteCommits(List<Commit> commits) {
    Map<RepoId, Set<Commit>> commitsByRepo =
        commits.stream().collect(groupingBy(Commit::getRepoId, toSet()));
    ...
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      ...
      for (Entry<RepoId, Set<Commit>> entry : commitsByRepo.entrySet()) {
        ...
        Map<String, Set<CommitHash>> allParentHashes =
            db
                .dsl()
        ...
        Map<String, Set<CommitHash>> allChildHashes =
            db
                .dsl()
                .selectFrom(COMMIT_RELATIONSHIP)
                .where(COMMIT_RELATIONSHIP.REPO_ID.eq(entry.getKey().getIdAsString()))
  ...
  /**
   * Gets all tracked commits whose <em>committer</em> date is between the given start and end time.
   *
   ...
   *     particular order
   */
  public List<Commit> getTrackedCommitsBetween(
      RepoId repoId, @Nullable Instant startTime, @Nullable Instant stopTime) {

    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      SelectConditionStep<KnownCommitRecord> query =
          db.dsl()
              .selectFrom(KNOWN_COMMIT)
              .where(KNOWN_COMMIT.REPO_ID.eq(repoId.getIdAsString()))
      ...
      return query.stream().map(CommitReadAccess::knownCommitRecordToCommit).collect(toList());
    }
  }
  ...
  public List<CommitHash> getFirstParentsOfBranch(
      RepoId repoId, BranchName branch, CommitHash rootHash) {
   ...
   *     order
   */
  public List<Commit> getCommitsBetween(
      RepoId repoId,
      Collection<BranchName> branchNames,
    ...
    List<String> branchNamesStr = branchNames.stream().map(BranchName::getName).collect(toList());

    String branchNamesBindings =
        branchNames.stream().map(s -> "?").collect(Collectors.joining(", "));
    ...
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      Set<String> reachableHashes =
          db.dsl().fetch(queryStr, bindings.toArray()).stream()
              .map(record -> record.get(0, String.class))
              .collect(toSet());
  ...
  public List<Pair<Commit, Boolean>> searchCommits(
      int limit, @Nullable RepoId repoId, String queryStr) {
    ...
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      ...
      return query.orderBy(KNOWN_COMMIT.COMMITTER_DATE.desc()).limit(limit).stream()
          .map(
              record ->
                  new Pair<>(
                      new Commit(
