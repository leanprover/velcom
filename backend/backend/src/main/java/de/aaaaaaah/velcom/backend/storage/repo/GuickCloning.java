...
public abstract class GuickCloning {
  ...
  private static GuickCloning findInstanceToUse() {
    try {
      ProgramResult git = new ProgramExecutor().execute(GIT_EXECUTABLE, "--version").get();
  ...
  private static class JgitCloning extends GuickCloning {
    ...
    public void cloneMirror(String source, Path targetDir) throws CloneException {
      try {
        CloneCommand cloneCommand =
            Git.cloneRepository().setURI(source).setDirectory(targetDir.toFile()).setBare(true);
    ...
    public void cloneCommit(String source, Path targetDir, String commitHash)
      ...
      try (Git clone =
        ...
        clone.clean().setCleanDirectories(true).setForce(true).call();
      } catch (GitAPIException e) {
    ...
    public void updateBareRepo(Path repoDir) throws CloneException {
      try {
        ...
        throw new CloneException("Error when pulling the latest changes from a bare git repo", e);
      }
    }
  }
  ...
  private static class CmdGitCloning extends GuickCloning {
    ...
    private void guardResult(String message, ProgramResult programResult) throws CloneException {
      ...
      throw new CloneException(
          message
              ...
              + "\nStdout: "
              + programResult.getStdOut()
              + "\nStderr: "
              + programResult.getStdErr()
              + "\nAfter: "
              + programResult.getRuntime());
    }
    ...
    public void cloneCommit(String source, Path targetDir, String commitHash)
      ...
      try {
        ProgramResult programResult =
            new ProgramExecutor()
                .execute(
                    ...
                    targetDir.toAbsolutePath().toString())
                .get();
        guardResult("Clone failed", programResult);
        ...
        guardResult("Checkout failed!", programResult);
        ...
        guardResult("Submodule checkout failed!", programResult);
      } catch (InterruptedException | ExecutionException e) {
