...
public class RecentRunsEndpoint {
  ...
  public GetReply getRuns(
    ...
    if (significant) {
      runEntries =
          significantRunsCollector.collectMostRecent(n).stream()
              .map(run -> toJsonRunEntry(run.getRun(), run.getReasons()))
              .collect(toList());
    } else {
      List<RunId> recentRunIds = benchmarkAccess.getRecentRunIds(0, n);
      runEntries =
          runCache.getRunsInOrder(benchmarkAccess, recentRunIds).stream()
