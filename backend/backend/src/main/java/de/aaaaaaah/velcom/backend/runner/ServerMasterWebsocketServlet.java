...
public class ServerMasterWebsocketServlet extends WebSocketServlet {
   ...
   */
  public ServerMasterWebsocketServlet(
      Dispatcher dispatcher, Serializer serializer, String runnerToken, BenchRepo benchRepo) {
  ...
  public void configure(WebSocketServletFactory factory) {
    ...
    factory.setCreator(
        (req, resp) -> {
          ...
          // So the dispatcher marks the runner as disposed and synchronizes on it. If the new
          // runner
          ...
          synchronized (myTeleRunner) {
            if (myTeleRunner.isDisposed()) {
              myTeleRunner = new TeleRunner(name, serializer, dispatcher, benchRepo);
              LOGGER.info(
                  "Revived runner {} connecting from {} with new instance!",
                  name,
  ...
  private void kickRunner(ServletUpgradeResponse response, RunnerDenyReason reason) {
    try {
      response.addHeader(
          RunnerConnectionHeader.DISCONNECT_DENY_REASON.getName(), reason.getHeaderValue());
