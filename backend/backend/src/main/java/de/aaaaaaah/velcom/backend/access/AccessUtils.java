...
public class AccessUtils {
   ...
   *
   * <p>If a commit hash is present, the source is a {@link CommitSource}, otherwise it is a {@link
   ...
   * @return the source
   */
  public static Either<CommitSource, TarSource> readSource(
      @Nullable String repoId, @Nullable String commitHash, @Nullable String tarDesc) {
    ...
    if (commitHash != null) { // Must be commit source
      ...
      return Either.ofLeft(new CommitSource(RepoId.fromString(repoId), new CommitHash(commitHash)));
      ...
      return Either.ofRight(
          new TarSource(tarDesc, Optional.ofNullable(repoId).map(RepoId::fromString).orElse(null)));
    }
  }
}
