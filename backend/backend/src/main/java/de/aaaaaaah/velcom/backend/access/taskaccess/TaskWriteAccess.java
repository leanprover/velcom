...
public class TaskWriteAccess extends TaskReadAccess {
   ...
   */
  public Optional<Task> insertCommit(
      String author, TaskPriority priority, RepoId repoId, CommitHash hash) {

    return databaseStorage.acquireWriteTransaction(
        db -> {
  ...
  public void insertCommits(
      String author, TaskPriority priority, RepoId repoId, Collection<CommitHash> hashes) {

    Set<String> hashesAsStrings =
        hashes.stream().map(CommitHash::getHash).collect(Collectors.toSet());

    databaseStorage.acquireWriteTransaction(
        db -> {
          Set<String> hashesAlreadyInQueue =
              db.dsl()
                  .selectFrom(TASK)
                  .where(TASK.REPO_ID.eq(repoId.getIdAsString()))
   ...
   *     list is in no particular order. If the selector returns a task, that task will be started
   *     and an updated version of it returned. The task returned by the selector <em>must be one of
   *     the input tasks</em>!
  ...
  public Optional<Task> startTask(Function<List<Task>, Optional<TaskId>> selector) {
    return databaseStorage.acquireWriteTransaction(
        db -> {
          List<Task> allTasks =
              db.dsl().selectFrom(TASK).where(not(TASK.IN_PROCESS)).stream()
                  .map(TaskReadAccess::taskRecordToTask)
                  .collect(Collectors.toList());

          Set<TaskId> allTaskIds = allTasks.stream().map(Task::getId).collect(Collectors.toSet());
          ...
          TaskRecord record =
              db.dsl().selectFrom(TASK).where(TASK.ID.eq(taskIdAsString)).fetchOne();
  ...
  public void deleteTasks(Collection<TaskId> tasks) {
    Set<String> taskIds = tasks.stream().map(TaskId::getIdAsString).collect(Collectors.toSet());

    try (DBWriteAccess db = databaseStorage.acquireWriteAccess()) {
      db.dsl().deleteFrom(TASK).where(TASK.ID.in(taskIds)).execute();
    }
  ...
  public void resetAllTaskStatuses() {
    try (DBWriteAccess db = databaseStorage.acquireWriteAccess()) {
      db.dsl().update(TASK).set(TASK.IN_PROCESS, false).execute();
    }
  }
  ...
  public void cleanUpTarFiles() throws IOException {
    ...
    try (DBReadAccess db = databaseStorage.acquireReadAccess()) {
      tarTaskIds =
          db.dsl().selectFrom(TASK).where(TASK.COMMIT_HASH.isNull()).stream()
