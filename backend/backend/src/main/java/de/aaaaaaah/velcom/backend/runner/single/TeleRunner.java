...
public class TeleRunner {
  ...
  public TeleRunner(
      String runnerName, Serializer serializer, Dispatcher dispatcher, BenchRepo benchRepo) {
  ...
  /**
   * Aborts the current benchmark. This is a very rudimentary implementation at the moment. <br>
   * The abort will do nothing, if
   *
   * <ul>
   *   <li>The runner is not connected
   *   <li>The send fails
   *   <li>The runner reconnects and has results for that run
   *   <li>The server sends a get_status before the abort and the runner has results
   * </ul>
   *
  ...
  public void prepareAndSendWork() {
    ...
    Optional<Task> workOptional =
        Optional.ofNullable(myCurrentTask.get()).or(() -> dispatcher.getWork(this));

    if (workOptional.isEmpty()) {
      LOGGER.debug("Dispatcher gave me no work for {}", getRunnerName());
      connection.send(new RequestRunReply(false, null, false, null).asPacket(serializer));
      return;
    }
    ...
    handleBinaryTransfer(
        task, outputStream -> dispatcher.getQueue().transferTask(task.getId(), outputStream));
  }
