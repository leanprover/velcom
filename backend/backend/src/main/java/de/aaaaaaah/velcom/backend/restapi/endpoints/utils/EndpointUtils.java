...
public class EndpointUtils {
  ...
  public static Run getRun(
    ...
    if (hash == null) {
      ...
      return latestRunCache
          .getLatestRun(benchmarkAccess, runCache, repoId, commitHash)
   ...
   * @param run the run
   * @param significanceFactors the current significance factors (required for stddev calculations)
   * @param allValues whether the full lists of values should also be included for each measurement
   * @return the created {@link JsonRun}
   */
  public static JsonRun fromRun(
    ...
    Map<Dimension, DimensionInfo> dimensionInfos =
        dimensionAccess.getDimensionInfoMap(run.getAllDimensionsUsed());
            ...
            .consume(
                ...
                it ->
                    JsonResult.fromMeasurements(
                        it, dimensionInfos, significanceFactors, allValues)));
  }
            ...
            s -> {
              ...
              if (elements.length == 0 || elements[0].equals("")) {
                ...
                throw new ArgumentParseException(
                    "section \"" + s + "\" needs at least two elements");
              }

              String sectionName = elements[0];
              List<String> sectionElements =
                  Arrays.stream(elements).skip(1).collect(Collectors.toList());
  ...
  public static Set<Dimension> parseDimensions(String args) {
    return parseColonSeparatedArgs(args).stream()
        .flatMap(
            pair -> pair.getSecond().stream().map(elem -> new Dimension(pair.getFirst(), elem)))
   ...
   */
  public static Pair<Instant, Instant> getStartAndEndTime(
      @Nullable Long startTimeEpoch,
      @Nullable Long endTimeEpoch,
   ...
   */
  public static List<FullCommit> topologicalSort(
      List<FullCommit> commits, Map<CommitHash, FullCommit> hashes) {
    ...
    Queue<FullCommit> leaves =
        commits.stream()
            // Only consider commits in our commits list. Throw away all children *not* in the list,
            // as
            // we are focusing on a small part of the graph and trying to sort only this part
            // topologically.
            // If we do not throw them away, we will find children outside the range, leading to too
            // few
            // found leaf nodes
    ...
    Map<FullCommit, Set<CommitHash>> parentChildMap =
        commits.stream()
            .collect(
                toMap(
                    it -> it,
                    it ->
                        it.getChildHashes().stream()
