...
public class Benchmarker {
  ...
  private final AtomicReference<Supplier<LinesWithOffset>>
      outputFetcher; // nullable inside the reference

  private final CompletableFuture<Boolean> finishFuture;

  private final UUID taskId;
  private final Path taskRepoPath;
  @Nullable private final String benchRepoHash;
   ...
   */
  public Benchmarker(
      CompletableFuture<Boolean> finishFuture,
      UUID taskId,
      Path taskRepoPath,
      @Nullable String benchRepoHash,
      Path benchRepoPath,
      Instant startTime,
      String runnerName,
  ...
  private void runBenchmark() {
    ...
    outputFetcher.set(
        () -> {
          ...
          List<String> sublist =
              lines.subList(Math.max(lines.size() - maxLinesToReturn, 0), lines.size());
  ...
  private StreamsProcessOutput<ProgramResult> startBenchExecution(
      NamedRows generalInfo, Path benchScriptPath) {
    Instant startTime = Instant.now();

    String[] calledCommand = {
      benchScriptPath.toAbsolutePath().toString(), taskRepoPath.toAbsolutePath().toString()
    };
  ...
  private void addProgramOutput(
      NamedRows generalInfo, NamedSections infoSections, ProgramResult programResult) {
    ...
    infoSections.addSection(
        "Stdout", programResult.getStdOut().isEmpty() ? "<empty>" : programResult.getStdOut());
    infoSections.addSection(
        "Stderr", programResult.getStdErr().isEmpty() ? "<empty>" : programResult.getStdErr());
  }
  ...
  private BenchResult interpretZeroExitCode(
      NamedSections infoSections, ProgramResult programResult) {
    ...
    try {
      ...
      infoSections.addSection("Reason", "The benchmark script returned invalid output!");
      return failedBenchResult(infoSections);
    }

    return successfulBenchResult(
        new Result(bareResult.getRight().orElse(null), bareResult.getLeft().orElse(null)));
  }

  private BenchResult interpretFailingExitCode(
      NamedSections infoSections, ProgramResult programResult) {
        ...
        .ifPresent(
            signal ->
                infoSections.addSection(
                    ...
                    "The exit code looks like the linux signal "
                        + signal.name()
                        + ".\n"
                        + "It's signal number is "
                        + signal.getNumber()
                        + " and it is caused by '"
  ...
  private BenchResult interpretExecutionException(
      NamedSections infoSections, ExecutionException e) {
    infoSections.addSection("Stacktrace", ExceptionHelper.getStackTrace(e));
    infoSections.addSection("End time", Instant.now().toString());
    infoSections.addSection(
        "Reason", "Maybe an internal runner error. Rebenchmarking might solve the problem!");
    return failedBenchResult(infoSections);
  }
