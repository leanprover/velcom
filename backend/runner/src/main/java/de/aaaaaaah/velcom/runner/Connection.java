...
public class Connection implements WebSocket.Listener, HeartbeatWebsocket {
  ...
  public Connection(
      TeleBackend teleBackend, HttpClient httpClient, URI address, String name, String token)
  ...
  @Override
  public synchronized CompletionStage<?> onClose(
      WebSocket webSocket, int statusCode, String reason) {

    String statusCodeStr =
        StatusCode.fromCode(statusCode).map(Enum::toString).orElse(Integer.toString(statusCode));
