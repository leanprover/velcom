...
public class NamedRows {
  ...
  public void addEscapedArray(String name, String[] array) {
    String entries =
        Arrays.stream(array).map(StringHelper::quote).collect(Collectors.joining(", "));
  ...
  public String format() {
    int maxNameLength =
        rows.stream().map(Row::getName).map(String::length).max(Integer::compareTo).orElse(0);

    return rows.stream().map(row -> row.format(maxNameLength)).collect(Collectors.joining());
  }
